...
- с `repair`, если основной код не применился или не прошел проверки;
- с успешным `generate-test`, если тестовый артефакт создан и прошел verification;
- с ошибкой вызова `generate-test`, если внешний генератор не вернул тестовый артефакт;
- с неуспешным generated test verification, если основной код корректен, но verification упал только на сгенерированном тесте.

Если внешний `generate-test` завершился ошибкой или не вернул тестовый артефакт, пайплайн не применяет тест, сохраняет диагностический `generated_test_apply.reason` и добавляет warning в run payload. Если verification не пройден, пайплайн может выполнить `repair`. Если после повторной проверки основной код проходит, а ошибка остается только в сгенерированном тесте, итоговый статус фиксируется как `generated_test_verification_failed`.
...
Если был сгенерирован новый тест, он может попадать в список рекомендуемых тестов.

Итог verification фиксируется отдельно от итогового статуса сценария.
...
Если `generate-test` не вернул тестовый артефакт из-за ошибки внешнего генератора, основной production-result не переводится автоматически в failure. Диагностика сохраняется в `generated_test_apply` и `warnings`, чтобы UI и человек явно видели, что тест не был создан.
...
- передает ограниченное число reference artifacts в `generate-test`, если они есть в context pack и разрешены настройкой `generate_test_reference_max_items`;
...
Используется в связанных сценариях, прежде всего в `generate-test`, когда тест строится уже по сгенерированному production-коду. Для `insert_after_symbol` сгенерированный symbol считается целевым объектом тестирования, а выбранный target остается anchor-symbol.

### RepairRequest

Используется для режима `repair`. При подготовке repair request `codecollector` сохраняет исходную requested operation, включая `insert_after_symbol`, и передает ее в `previous_artifact.operation`, чтобы repair не превращал вставку нового symbol в замену anchor-symbol.
...
Текущий `session.status` отражает актуальное состояние сценария.

Основные состояния session:
...
- качество `generate-test` может приводить к статусу `generated_test_verification_failed`, даже если основной код корректен.
---
